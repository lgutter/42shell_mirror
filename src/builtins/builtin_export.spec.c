...
Test(builtin_export_unit, invalid_key_bad_format, .init = redirect_std_err)
{
	char		**argv;
...
// Test(builtin_export_unit, invalid_deny_read_only_overwrite, .init = redirect_std_err)
// {
// 	char		**argv;
// 	t_shell		*shell = init_shell(false);
// 	int			ret = 0;
...
// Test(builtin_export_unit, valid_force_read_only_overwrite)
// {
// 	char		**argv;
// 	t_shell		*shell = init_shell(false);
// 	int			ret = 0;
...
Test(builtin_export_unit, valid_normal_overwrite)
{
	char		**argv;
	t_shell		*shell = init_shell(false);
	int			ret = 0;
...
Test(builtin_export_unit, valid_normal_create_var)
{
	char		**argv;
	t_shell		*shell = init_shell(false);
	int			ret = 0;
...
Test(builtin_export_unit, valid_different_var_type)
{
	char		**argv;
	t_shell		*shell = init_shell(false);
	int			ret = 0;

	ft_setenv(shell->env, "SAMENAME", "INSHELLVARS", (RO_VAR | SHELL_VAR));
	argv = (char **)malloc(sizeof(char *) * 3);
	argv[0] = ft_strdup("export");
	argv[1] = ft_strdup("SAMENAME=INENVVARS");
	argv[2] = NULL;
	ret = builtin_export(shell, argv);
	cr_expect_eq(ret, 0, "ret is %d but must be %d", ret, 0);
...
Test(builtin_export_unit, invalid_invalid_option, .init = redirect_std_err)
{
	char		**argv;
	t_shell		*shell = init_shell(false);
	int			ret = 0;
...
Test(builtin_export_unit, valid_env_no_arguments, .init = redirect_std_out)
{
	char		**argv;
...
Test(builtin_export_unit, invalid_NULL_argv)
{
	char		**argv;
	t_env		shell_var = {"baz", "blah", SHELL_VAR, NULL};
	t_env		env = {"foo", "bar", ENV_VAR, &shell_var};
...
Test(builtin_export_unit, invalid_NULL_command)
{
	t_env		shell_var = {"baz", "blah", SHELL_VAR, NULL};
	t_env		env = {"foo", "bar", ENV_VAR, &shell_var};
	t_env		*start = &env;
