...
Test(builtin_bg_unit, basic_no_args_current_job, .init =cr_redirect_stdout)
{
	...
	int stat_loc;
	usleep(100000);
	pid_t ret = waitpid(job->pgrp, &stat_loc, WNOHANG | WUNTRACED);
	cr_assert_eq(ret, job->pgrp, "job was not suspended!");
	cr_expect_eq(0, builtin_bg(shell, argv));
	ret = waitpid(job->pgrp, &stat_loc, WNOHANG | WCONTINUED);
...
Test(builtin_bg_unit, basic_match_all_jobs, .init =cr_redirect_stdout)
{
	...
	pid_t ret = waitpid(job->pgrp, &stat_loc, WNOHANG | WUNTRACED);
	cr_assert_eq(ret, job->pgrp, "job was not suspended!");
	ret = waitpid(job2->pgrp, &stat_loc, WNOHANG | WUNTRACED);
	cr_assert_eq(ret, job2->pgrp, "second job was not suspended!");
	cr_expect_eq(0, builtin_bg(shell, argv));
	ret = waitpid(job->pgrp, &stat_loc, WNOHANG | WCONTINUED);
	cr_expect_eq(ret, job->pgrp, "job was not continued by bg!");
...
Test(builtin_bg_unit, basic_match_two_jobs, .init =cr_redirect_stdout)
{
	...
	ret = waitpid(job2->pgrp, &stat_loc, WNOHANG | WUNTRACED);
	cr_assert_eq(ret, job2->pgrp, "second job was not suspended!");
	ret = waitpid(job3->pgrp, &stat_loc, WNOHANG | WUNTRACED);
	cr_assert_eq(ret, job3->pgrp, "third job was not suspended!");
	cr_expect_eq(0, builtin_bg(shell, argv));
	ret = waitpid(job->pgrp, &stat_loc, WNOHANG | WCONTINUED);
	cr_expect_eq(ret, job->pgrp, "job was not continued by bg!");
	ret = waitpid(job2->pgrp, &stat_loc, WNOHANG | WCONTINUED);
